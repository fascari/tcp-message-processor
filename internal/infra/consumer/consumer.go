...
func (c Consumer) saveStatistics(ctx context.Context, event server.Event) error {
...
func unmarshalEvent(body []byte) (server.Event, error) {
	var event server.Event
	if err := json.Unmarshal(body, &event); err != nil {
		return server.Event{}, fmt.Errorf("failed to unmarshal event: %w", err)
	}
	return event, nil
}
